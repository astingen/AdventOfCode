...
namespace AdventOfCode.Utils
{
	public static class EnumUtils
	{
		...
		public static IEnumerable<object> GetValues(Type type)
		{
			...
			Monitor.Enter(s_EnumValuesCache);
			try
			{
			...
			{
				if(Monitor.IsEntered(s_EnumValuesCache))
					Monitor.Enter(s_EnumValuesCache);
			}
		}
		...
		public static IEnumerable<object> GetFlags(Type type, object value)
		{
			...
			Monitor.Enter(s_EnumFlagsCache);
			try
			{
				return s_EnumFlagsCache
			...
			{
				if (Monitor.IsEntered(s_EnumFlagsCache))
					Monitor.Exit(s_EnumFlagsCache);
			}
		}
			...
			where T : struct, IConvertible
		{
			...
			string message = string.Format("Failed to parse {0} as {1}", data, typeof(T).Name);
		...
		public static int Parse(Type type, string data, bool ignoreCase)
		{
			...
			string message = string.Format("Failed to parse {0} as {1}", data, type.Name);
			throw new FormatException(message);
		}
			...
			where T : struct, IConvertible
		{
			...
			catch (Exception e)
			{
				throw new FormatException(
					string.Format("Failed to parse {0} as {1}", data, typeof(T).Name), e);
			}
		...
		public static int ParseStrict(Type type, string data, bool ignoreCase)
		{
			...
			{
				throw new FormatException(
					string.Format("Failed to parse {0} as {1}", data, type.Name), e);
			}
